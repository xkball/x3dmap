...
void main() {
    vec4 view0 = ModelViewMat * vec4(P0, 1.0);
    vec4 view1 = ModelViewMat * vec4(P1, 1.0);

    vec4 clip0 = ProjMat * view0;
    vec4 clip1 = ProjMat * view1;
    ...
    if (aBehind != bBehind) {
        float t = (-clip0.w - clip0.z)
        / ((clip1.z + clip1.w) - (clip0.z + clip0.w));

        vec4 clipP = mix(clip0, clip1, t);
        vec4 viewP = mix(view0, view1, t);
